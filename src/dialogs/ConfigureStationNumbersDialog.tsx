...
interface ConfigureStationNumbersDialogProps {
  open: boolean;
  onClose: () => void;
  activityCode: string;
}

const ConfigureStationNumbersDialog = ({
  open,
  onClose,
  activityCode,
}: ConfigureStationNumbersDialogProps) => {
      ...
      .flatMap((p) => {
        const assigments: ExtendedAssignment[] | undefined = p.assignments
          ?.map((a) => {
            ...
            return {
              ...a,
              activity,
              room: roomByActivity(currentWcif, a.activityId),
        ...
        if (competitorAssignment) {
          a.push({
            ...
            seedResult: getSeedResult(currentWcif, activityCode, p),
          });
        }
  ...
  const arbitrarilyAssignStationNumbers = (assignmentCode: string) => {
    ...
    const lastStationNumberMap = new Map<number, number>();
  ...
  const handleCellEditStop: GridEventListener<'cellEditStop'> = (params, event) => {
    const row = params.row as StationRow;
    const input = (event as MuiEvent<React.SyntheticEvent>).target as HTMLInputElement | null;
    const stationNumber = input?.value ? parseInt(input.value, 10) : NaN;
