...
interface CheckFirstTimerDialogProps {
  open: boolean;
  onClose: () => void;
  person?: Person;
}

export default function CheckFirstTimerDialog({
  open,
  onClose,
  person,
}: CheckFirstTimerDialogProps) {
  const [personSearch, setPersonSearch] = useState<WcaPersonSearchResult[] | null>(null);
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg">
      <AppBar sx={{ position: 'relative' }} color="inherit">
        <Toolbar>
          <Avatar src={person?.avatar?.thumbUrl || person?.avatar?.url || ''} />
      ...
      <DialogContent style={{ paddingTop: 0 }}>
        ...
        <List>
          {personSearch?.map(({ person: p, competition_count }) => (
            <ListItemButton key={p.id} component="a" href={p.url} target="_blank">
              <ListItemIcon>
