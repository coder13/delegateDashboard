...
const PersonsAssignmentsDialog = ({
  ...
  const groupActivities = useMemo<ActivityWithParent[]>(
    () => (wcif ? (findGroupActivitiesByRound(wcif, roundId) as ActivityWithParent[]) : []),
    [wcif, roundId]
  );
              ...
              <TableRow key={person.registrantId}>
                ...
                <TableCell>
                  {activitiesByPersonAndAssignmentCode(person, 'competitor')
                    .map((activity) => {
                      const roomName = activity.parent?.room?.name || 'Unknown';
                      return `${roomName}: ${parseActivityCode(activity.activityCode).groupNumber}`;
                    })
                    .join(', ')}
                </TableCell>
                <TableCell>
