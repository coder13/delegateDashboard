...
const TableButton = styled(TableCell, { shouldForwardProp: (prop) => prop !== 'assignmentColor' })(({ theme, assignmentColor }) => (`
  text-align: center;
  ...
  border-radius: 0px;
  transition: ${theme.transitions.create(['background-color'], { easing: theme.transitions.easeInOut, duration: theme.transitions.duration.shortest })};
...
function TableAssignmentCell({ value, onClick }) {
  return (
    <TableButton
      onClick={onClick}
      assignmentColor={value && assignmentMap[value]?.color}
    >
